        ...
        btn_register.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (username.length() == 0 || password.length() == 0 || tmp.length() == 0) {
                    ToastUtil.show(AccountRegisterService.this.context, "用户名密码不能为空");
                    return;
                }

                if (!password.equals(tmp)) {
                    ToastUtil.show(AccountRegisterService.this.context, "两次输入的密码不一致");
                    return;
                }

                if (password.length() < 8) {
                    ToastUtil.show(AccountRegisterService.this.context, "密码的长度不能小于8个字符");
                }

//                int result = tlsService.TLSStrAccReg(username, password, strAccRegListener);
//                if (result == TLSErrInfo.INPUT_INVALID) {
//                    ToastUtil.show(AccountRegisterService.this.context, "帐号不合法");
//                }
            }
        });
    }
    ...
    class StrAccRegListener implements TLSStrAccRegListener {
        ...
        public void OnStrAccRegSuccess(TLSUserInfo userInfo) {
            ToastUtil.show(context, "成功注册 " + userInfo.identifier);
